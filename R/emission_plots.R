...
Intensity_Plot <- function(years=c(2050, 2100), regions="World", year0=2010, scenplot=scenlist, animate_plot=FALSE){
 ...
 tpes <- get_witch("tpes"); tpes_IP <- tpes %>% mutate(value=value*0.0036) %>% rename(PES=value)
  ...
  if(regions[1]=="World"){
    ...
    # Create region-specific palette that only includes regions in the data
    regions_in_data <- unique(Intensity_t$n)
    region_palette_filtered <- get_region_palette(regions_in_data, reg_id = if(exists("reg_id")) reg_id else NULL)
    p_imp <- ggplot() + geom_point(data=Intensity_t, mapping=aes(x=CI_change, y=EI_change, colour=n, shape=file), size=6) + geom_hline(linewidth=1,aes(yintercept=-.011), linetype="dashed") + geom_vline(linewidth=1,aes(xintercept=-0.003), linetype="dashed") + xlab(paste0("Carbon Intensity Change p.a.")) + ylab(paste0("Energy Intensity Change p.a.")) + guides(color=guide_legend(title=NULL, nrow = 2)) + theme(legend.position="bottom", legend.direction = "horizontal", legend.box = "horizontal") + scale_x_continuous(labels=scales::percent) + scale_y_continuous(labels=scales::percent) + scale_color_manual(values = region_palette_filtered)
    p_ciei <- ggplot() + geom_point(data=subset(Intensity_t), mapping=aes(x=CI, y=EI, color=n, shape=file), size=6) + xlab(paste0("Carbon Intensity [gCO2eq/MJ]")) + ylab(paste0("Energy Intensity [MJ/$]")) + guides(color=guide_legend(title=NULL), shape=guide_legend(title=NULL)) + theme(legend.position="bottom", legend.direction = "horizontal", legend.box = "horizontal") + scale_color_manual(values = region_palette_filtered)
    }
...
ggplot(subset(Q_EMI_SECTORS, t<=10 & n %in% regions & sector=="Fossil Fuels and Industrial" & file %in% scenplot)) + geom_line(stat="identity", linewidth=1.2, aes(ttoyear(t),value, color=file)) + facet_wrap( ~ n, scales = "free", switch=NULL, ncol=length(regions)) + ylab("GtCO2") + xlab("") + guides(color=guide_legend(title=NULL, nrow = 2)) + theme(legend.position="bottom")
saveplot("Sectoral CO2 Emissions FFI")
ggplot(subset(Q_EMI_SECTORS, t<=10 & n %in% regions & sector=="Land Use" & file %in% scenplot)) + geom_line(stat="identity", linewidth=1.2, aes(ttoyear(t),value, color=file)) + facet_wrap( ~ n, scales = "free", switch=NULL, ncol=length(regions)) + ylab("GtCO2") + xlab("") + guides(color=guide_legend(title=NULL, nrow = 2)) + theme(legend.position="bottom")
