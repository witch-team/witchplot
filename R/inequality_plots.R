...
compute_global_inequality <- function(Y_DIST="Y_DIST", Y="Y", l="l", scenplot=scenlist){
  ...
  # Try to load from results_dir first
  hist_quantiles_file <- file.path(results_dir, "data_historical_values_inequality_converted.gdx")
  hist_values_file <- file.path(results_dir, "data_historical_values.gdx")

  if(!file.exists(hist_quantiles_file) || !file.exists(hist_values_file)) {
    warning(sprintf("Inequality historical data files not found in results_dir.\nPlease place the following files in '%s':\n  - data_historical_values_inequality_converted.gdx\n  - data_historical_values.gdx\nSkipping historical inequality data.", results_dir))
    # Return only model data without historical
    return(inequality_dataset_model)
  }

  data_hist_quantiles <- gdx(hist_quantiles_file)
  data_hist_quantiles <- data_hist_quantiles["quantiles"] %>% mutate(year=as.numeric(year))
  data_historical <- gdx(hist_values_file)
